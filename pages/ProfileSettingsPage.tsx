...
import { IconEdit, IconUser, IconLockClosed, IconPhoto, IconChevronLeft, IconChevronRight, IconChevronDown } from '../constants';
...
const MobileNav: React.FC<{
    ...
    navItems: any[];
}> = ({ activeSection, onSectionChange, navItems }) => {
    ...
    return (
        <div ref={navRef} className="relative">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between text-left px-4 py-2 rounded-lg bg-surface-nested border border-border-color">
                <span className="font-semibold text-text-primary">{activeSection}</span>
            ...
            {isOpen && (
                <div className="absolute top-full left-0 w-full bg-surface rounded-lg shadow-lg mt-1 z-20 border border-border-color">
                    <ul className="py-1">
                        {navItems.map(item => (
                            <li key={item.name}>
                                <button onClick={() => { onSectionChange(item.name); setIsOpen(false); }} className={`w-full text-left px-4 py-2 text-sm font-medium ${activeSection === item.name ? 'text-primary bg-primary/10' : 'text-text-strong'}`}>
                                    {item.name}
                                </button>
                            </li>
                        ))}
        ...
        <div className="flex h-full bg-background">
            ...
            <main className="flex-1 overflow-y-auto">
                <div className="lg:hidden p-4 border-b border-border-color bg-surface sticky top-0 z-10">
                    <MobileNav activeSection={activeSection} onSectionChange={onSectionChange} navItems={settingsNavItems} />
                </div>
                <div className="p-4">
