...
const QueryPreviewModal: React.FC<QueryPreviewModalProps> = ({ query, isOpen, onClose, onAnalyze, onOptimize, onSimulate }) => {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={`Query Preview: ${query.id.substring(7, 13).toUpperCase()}`} size="lg">
            <div className="flex flex-col h-full">
                ...
                <div className="p-6 bg-background mt-auto flex justify-end items-center gap-3 flex-shrink-0">
                    <button onClick={() => { onAnalyze(query); onClose(); }} className="flex items-center gap-2 text-sm font-semibold px-4 py-2 rounded-full border border-border-color hover:bg-gray-50" title="Open this query in Analyzer to inspect performance details."><IconSearch className="h-4 w-4" /> Open in Analyzer</button>
                    <button onClick={() => { onOptimize(query); onClose(); }} className="flex items-center gap-2 text-sm font-semibold px-4 py-2 rounded-full border border-border-color hover:bg-gray-50" title="Open in Optimizer to get AI-powered optimization suggestions."><IconWand className="h-4 w-4" /> Open in Optimizer</button>
                    <button onClick={() => { onSimulate(query); onClose(); }} className="flex items-center gap-2 text-sm font-semibold px-4 py-2 rounded-full border border-border-color hover:bg-gray-50" title="Open in Simulator to test cost and performance scenarios."><IconBeaker className="h-4 w-4" /> Open in Simulator</button>
                </div>
            </div>
        </Modal>
    );
